...
void readSaveFiles() {
  ...
  ///////////
    ...
    latex.DrawLatex(0.4, 0.83, "#font[132]{#it{N(t) = N_{0} e^{-t /#tau} + N_{1}}}");
    latex.DrawLatex(0.4, 0.75, Form("#font[132]{#it{#tau} = (%.3f #pm %.3f) #it{#mu}s}", fitFunc->GetParameter(1), fitFunc->GetParError(1)));
    latex.DrawLatex(0.4, 0.67, Form("#font[132]{#it{N_{0}} = %.3f #pm %.3f}", fitFunc->GetParameter(0), fitFunc->GetParError(0)));
    latex.DrawLatex(0.4, 0.59, Form("#font[132]{#it{N_{1}} = %.3f #pm %.3f}", fitFunc->GetParameter(2), fitFunc->GetParError(2)));
    latex.DrawLatex(0.4, 0.51, Form("#font[132]{#it{#chi^{2}} / ndf = %.3f}", chi2 / ndf));
  ////////////////
